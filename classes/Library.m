classdef Library < BetterHandle
    ...
    methods
        ...
        function load_zh_library(this, data)
            %% shape
            % mu: (200, 10)
            this.shape.mu = data.mu;
            % Sigma: (10, 10, 200)
            Sigma = zeros(200, 10, 10);
            for i=1:size(data.sigma, 1)
                Sigma(i, :, :) = diag(data.sigma(i, :));
            end
            this.shape.Sigma = permute(Sigma, [2, 3, 1]);
            % vsd: (200, 10)
            this.shape.vsd = sqrt(data.sigma);
            % mixprob: (1, 200)
            this.shape.mixprob = data.mixprob;
            % freq: (1, 200)
            this.shape.freq = data.freq;
            
            %% scale
            % theta: (200, 2)
            this.scale.theta = data.theta;
            
            %% rel
            % sigma_x: (1, 1) = 1.3536
            % sigma_y: (1, 1) = 1.2056
            % mixprob: (4, 1) = [0.3356; 0.0549; 0.1053; 0.5042]
            ...
            %% tokenvar
            % sigma_shape: (1, 1) = 3.3593;
            % sigma_invscale: (1, 1) = 0.0277;
            % sigma_attach: (1, 1) = 0.1499;
            
            %% affine
            % mu_scale: (1, 2) = [1, 1];
            % mu_xtranslate: (1, 1) = 0;
            % mu_ytranslate: (1, 1) = 0;
            % Sigma_scale: (2, 2) = [0.0470, 0.0223; 0.0223, 0.0359];
            % sigma_xtranslate: 5.2658;
            % sigma_ytranslate: 5.0867;
            
            %% stat (empty)
            this.stat.q_ink_emp = [];
            this.stat.q_ink_prior = [];
            this.stat.q_canvas_emp = [];
            this.stat.q_canvas_prior = [];
            
            %% Others
            % logT: (200, 200)
            this.logT = data.logT;
            % logStart: (200, 1)
            this.logStart = data.logStart';
            % pkappa: (15, 1)
            this.pkappa = data.pkappa';
            % pmat_nsub: (15, 15)
            this.pmat_nsub = data.pmat_nsub;
            % newscale: (1, 1) = 105
            this.newscale = 105;
            % smooth_bigrams: (1, 1) = 0.1000;
            % diagSigma: bool = 1;
            % ncpt: (1, 1) = 5;
            % N: (1, 1)
            this.N = 200;
